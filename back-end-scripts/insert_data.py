#### SCRIPT TO INSERT GTFS DATA INTO SQLITE DATABASE : WILL FILL THE TUB.sqlite3 file ####
import pandas as pd
import numpy as np
import sqlite3
from mapping_database import db, Routes, Trips, Stops, Transfers, Stop_times, Stops_routes, Timetable
...
def insert_data (Class):
    object_list = []
    gtfsfile = pd.read_csv("data/GTFS_Bergerac/%s.txt" % Class.__tablename__)
...
### INSERTING DATA FROM GTFS FILES INTO ROUTES, TRIPS, STOPS, STOP_TIMES AND TRANSFERS MAPPED TABLE

insert_data(Routes);insert_data(Trips);insert_data(Stops);insert_data(Transfers);insert_data(Stop_times)

### INSERTING DATA INTO STOP_ROUTES TABLES : RETRIEVING ALL ROUT ID FOR EACH STOP ID
